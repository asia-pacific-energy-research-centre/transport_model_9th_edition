...
default_save_folder = f'plotting_output/{subfolder_name}/{FILE_DATE_ID}/'
...
#add occupancy, load, passenger_km, freight_tonne_km to the dict
measure_to_unit_concordance_dict['Occupancy'] = 'passengers'
measure_to_unit_concordance_dict['Load'] = 'tonnes'
measure_to_unit_concordance_dict['passenger_km'] = 'billion passenger-km'  
measure_to_unit_concordance_dict['freight_tonne_km'] = 'billion tonne-km'
#%%
...
def log_time(section, elapsed_time):
    ...
    times_df = times_df.append({'section': section, 'time': elapsed_time, 'num_economies': len(economies_to_filter_for)}, ignore_index=True)
...
def print_expected_time_to_run(section):
    try:
        ...
        times_df = times_df[times_df['num_economies']==len(economies_to_filter_for)]
        ...
        for value_col in value_cols:
            for i in range(1, n_categorical_cols+1):
                for combo in itertools.combinations(categorical_cols, i):
                    ...
                    title = f'{value_col} by {list(combo) + ["Fuel"]}'
                    save_folder = f'{default_save_folder}/{dataframe_name}/{economy_x}/{value_col}'
