...
transport_dataset.drop(['Unit','Dataset_selection_method','Dataset', 'Data_available'], axis=1, inplace=True)
...
road_model_input_wide['Travel_km'] = np.nan
road_model_input_wide.loc[road_model_input_wide['Transport Type'] == 'passenger', 'Travel_km'] = road_model_input_wide['passenger_km']/road_model_input_wide['Occupancy']
road_model_input_wide.loc[road_model_input_wide['Transport Type'] == 'freight', 'Travel_km'] = road_model_input_wide['freight_tonne_km']/road_model_input_wide['Load']
...
# road_model_input_wide['Stocks'] = road_model_input_wide['Travel_km'] / road_model_input_wide['Travel_km_per_stock']#calculate stocks again? #TODO is this really the best wayt to do thinga?

# #quick analysis. lets average the travel km per stock for each vehicle type and see what the average is.
# road_model_input_wide[['Travel_km_per_stock', 'Transport Type', 'Vehicle Type']].groupby(['Transport Type', 'Vehicle Type']).agg([np.mean, np.std, np.var, np.median]).reset_index()
################################################################################
#%%
quick_fix = True
if quick_fix == True:
...
if not quick_fix:
    ...
    if len(road_model_input_wide_correct_eff) == 0:
        ...
        road_model_input_wide.loc[road_model_input_wide['Transport Type'] == 'passenger', 'passenger_km'] = road_model_input_wide['Travel_km'] * road_model_input_wide['Occupancy']
        road_model_input_wide.loc[road_model_input_wide['Transport Type'] == 'freight', 'freight_tonne_km'] = road_model_input_wide['Travel_km'] * road_model_input_wide['Load']
...
non_road_model_input_wide.loc[non_road_model_input_wide['Transport Type'] == 'passenger', 'Efficiency'] = non_road_model_input_wide['passenger_km']/non_road_model_input_wide['Energy']
non_road_model_input_wide.loc[non_road_model_input_wide['Transport Type'] == 'freight', 'Efficiency'] = non_road_model_input_wide['freight_tonne_km']/non_road_model_input_wide['Energy']
...
#where transport type is freight, activity is freight_tonne_km, where transport type is passenger activity is  passenger_km
non_road_model_input_wide_avg_eff.loc[non_road_model_input_wide_avg_eff['Transport Type'] == 'passenger', 'Efficiency_avg'] = non_road_model_input_wide_avg_eff['passenger_km']/non_road_model_input_wide_avg_eff['Energy']
non_road_model_input_wide_avg_eff.loc[non_road_model_input_wide_avg_eff['Transport Type'] == 'freight', 'Efficiency_avg'] = non_road_model_input_wide_avg_eff['freight_tonne_km']/non_road_model_input_wide_avg_eff['Energy']
