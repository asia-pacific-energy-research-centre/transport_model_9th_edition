...
#######################################################################
#%%
#load in model_data.to_pickle('intermediate_data/archive/model_data_{}.pkl'.format(FILE_DATE_ID))
model_data = pd.read_pickle('intermediate_data/archive/road_model_data_{}.pkl'.format(FILE_DATE_ID))

#############################
#%%
#LOGISITC FUNCTION:
# Fit the logistic function to your data
# popt, pcov = curve_fit(logistic, x_data, y_data, bounds=(0, [gamma, 3., max(x_data)]))
parameters_estimates = logistic_parameters_estimation_function.logistic_fitting_function_handler(model_data,
BASE_YEAR = 2020,future_dates=[2045, 2050], future_dates_prop_diff_from_gamma=[0.1, 0.09], INTERPOLATE_DATA = True,show_plots=False)
#TEST PARAMS:
# # Parameters
# gamma = 100  # Maximum value (gamma)
# growth_rate = 1    # Growth rate
# midpoint = 90  # Midpoint

# logistic_parameters_estimation_function.plot_logistic_fit(date, vehicle_ownership_rates_data, gdp_per_capita_fit, gamma, growth_rate, midpoint, economy_vtype_scenario,show_plots, matplotlib_bool=True, plotly_bool=True)

#%%
#LOGISITC FUNCTION:
# Fit the logistic function to your data
# popt, pcov = curve_fit(logistic, x_data, y_data, bounds=(0, [gamma, 3., max(x_data)]))
gdp_per_capita_fit, vehicle_ownership_rates_data, gamma,economy_vtype_scenario,show_plots = logistic_parameters_estimation_function.logistic_fitting_function_handler(model_data,BASE_YEAR,future_dates=[2045, 2050], future_dates_prop_diff_from_gamma=[0.1, 0.09], INTERPOLATE_DATA = True,show_plots=False)
#%%
...
# Fit the logistic function to your data
popt, pcov = curve_fit(logistic, gdp_per_capita_fit, vehicle_ownership_rates_data, bounds=(0, [gamma, 3., max(gdp_per_capita_fit)]))

# Use the fitted function to calculate growth
gamma, growth_rate, midpoint = popt
projected_growth = logistic(x_data, gamma, growth_rate, midpoint)

plot_logistic_fit(date, vehicle_ownership_rates_data, gdp_per_capita_fit, gamma, growth_rate, midpoint, economy_vtype_scenario,show_plots, matplotlib_bool=True, plotly_bool=True)
# %%
...
for economy in model_data['Economy'].unique():
    for scenario in model_data['Scenario'].unique():
        for vehicle_type in model_data['Vehicle Type'].unique():
            ...
            model_data_economy_scenario_vtype['Stocks_per_thousand_capita'] = model_data_economy_scenario_vtype['Thousand_stocks_per_capita'] * 100000
