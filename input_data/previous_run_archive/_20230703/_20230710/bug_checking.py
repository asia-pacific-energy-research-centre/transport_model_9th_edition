...
for key, grp in change_dataframe_aggregation_nz_lv.groupby(['Drive']):
    ax = grp.plot(ax=ax, kind='line', x='Date', y='Activity_growth', label=key)
...
# change_dataframe_aggregation_growth = change_dataframe_aggregation[['Date', 'Economy', 'Transport Type','Activity_growth']].drop_duplicates()

# #sum up the activity data for each eyar and economy then calcualte the pct change per Date
# change_dataframe_aggregation_growth2 = change_dataframe_aggregation[['Date', 'Economy','Transport Type', 'Activity']].groupby(['Economy', 'Transport Type','Date']).sum().reset_index()

# #now create a new column with the pct change
# #sort by Date and everything else in ascending order
# change_dataframe_aggregation_growth2 = change_dataframe_aggregation_growth2.sort_values(by=['Economy', 'Transport Type', 'Date'])

# #calc growth rate. set index so that the growth rate is calc only for Value col
# change_dataframe_aggregation_growth3 = change_dataframe_aggregation_growth2.set_index(['Economy','Transport Type', 'Date']).pct_change()

# #now set all vlaues during the base Date (plus one cause this dtf ddoesnt have 2017 data) to 0 as the growth rate is not defined for the base Date
# if BASE_Date == 2017:
#     change_dataframe_aggregation_growth3.loc[change_dataframe_aggregation_growth3.index.get_level_values('Date') == BASE_Date+1, 'Activity'] = 0
# else:
#     change_dataframe_aggregation_growth3.loc[change_dataframe_aggregation_growth3.index.get_level_values('Date') == BASE_Date, 'Activity'] = 0
...
# activity_growth = change_dataframe_aggregation_growth3.merge(change_dataframe_aggregation_growth2, on=['Economy', 'Transport Type','Date'], how='left')

# #merge onto the original dataframe
# change_dataframe_aggregation_growth = change_dataframe_aggregation_growth.merge(activity_growth, on=['Economy','Transport Type', 'Date'], how='left')
...
# change_dataframe_aggregation_growth_2017_2021 = change_dataframe_aggregation_growth[(change_dataframe_aggregation_growth['Date'].isin([2017, 2018, 2019, 2020, 2021]))]

# #IT SEEMS THAT ACTIVITY GROWTH IS BVEING CALCULATED KINDA WEIRD. LETS TRY A DIFFERENT WAY BY LETTING GITHUB COPILOT DO IT
# change_dataframe_aggregation_growth4 = change_dataframe_aggregation[['Date', 'Economy','Transport Type', 'Activity']].groupby(['Economy', 'Transport Type','Date']).sum().reset_index()

# #now calcualte percenta change differently
# change_dataframe_aggregation_growth4 = change_dataframe_aggregation_growth4.sort_values(by=['Economy', 'Transport Type', 'Date'])


# change_dataframe_aggregation_growth4['Percentage_change'] = change_dataframe_aggregation_growth4['Activity'].pct_change()

# #set base Date values to 0
# if BASE_Date == 2017:
#     change_dataframe_aggregation_growth4.loc[change_dataframe_aggregation_growth4['Date'] == BASE_Date+1, 'Percentage_change'] = 0
# else:
#     change_dataframe_aggregation_growth4.loc[change_dataframe_aggregation_growth4['Date']== BASE_Date, 'Percentage_change'] = 0
...
# change_dataframe_aggregation_growth = change_dataframe_aggregation_growth.merge(change_dataframe_aggregation_growth4, on=['Economy', 'Transport Type','Date'], how='left') 
# #%%
# #sdet the dataframe to the one for early Dates
# # change_dataframe_aggregation_growth = change_dataframe_aggregation_growth_2017_2021.copy()
# #now plot it all
# title = 'Original activity growth vs activity vs calculated activity growth early Dates'
...
#             fig.add_trace(go.Scatter(x=model_output_detailed_lv_passenger_economy_ttype['Date'], y=model_output_detailed_lv_passenger_economy_ttype['Activity'],  legendgroup=legend_name, name=legend_name, line=dict(color=color1, width=2, )), row=row_number, col=col_number, secondary_y=False)

#             #create subplot for this economy
#             legend_name = 'Calculated_activity_growth_'+transport_type
#             fig.add_trace(go.Scatter(x=model_output_detailed_lv_passenger_economy_ttype['Date'], y=model_output_detailed_lv_passenger_economy_ttype['Calculated_activity_growth'],  legendgroup=legend_name, name=legend_name, line=dict(color=color2, width=2, )), row=row_number, col=col_number, secondary_y=True)

#             legend_name = 'Activity_growth_'+transport_type
#             fig.add_trace(go.Scatter(x=model_output_detailed_lv_passenger_economy_ttype['Date'], y=model_output_detailed_lv_passenger_economy_ttype['Activity_growth'], legendgroup=legend_name, name=legend_name, line=dict(color=color3, dash='dot', width=2)), row=row_number, col=col_number, secondary_y=True)

#             legend_name = 'Percentage_change_'+transport_type
#             fig.add_trace(go.Scatter(x=model_output_detailed_lv_passenger_economy_ttype['Date'], y=model_output_detailed_lv_passenger_economy_ttype['Percentage_change'], legendgroup=legend_name, name=legend_name, line=dict(color=color4, dash='dot', width=2)), row=row_number, col=col_number, secondary_y=True)

#         else:#legend is already set, so just add the traces with showlegend=False
#             #create subplot for this economy
#             legend_name = 'Activity_'+transport_type
#             fig.add_trace(go.Scatter(x=model_output_detailed_lv_passenger_economy_ttype['Date'], y=model_output_detailed_lv_passenger_economy_ttype['Activity'],  legendgroup=legend_name, name=legend_name,showlegend=False, line=dict(color=color1, width=2, )), row=row_number, col=col_number, secondary_y=False)

#             #create subplot for this economy
#             legend_name = 'Calculated_activity_growth_'+transport_type
#             fig.add_trace(go.Scatter(x=model_output_detailed_lv_passenger_economy_ttype['Date'], y=model_output_detailed_lv_passenger_economy_ttype['Calculated_activity_growth'],  legendgroup=legend_name, name=legend_name, showlegend=False, line=dict(color=color2, width=2, )), row=row_number, col=col_number, secondary_y=True)

#             legend_name = 'Activity_growth_'+transport_type
#             fig.add_trace(go.Scatter(x=model_output_detailed_lv_passenger_economy_ttype['Date'], y=model_output_detailed_lv_passenger_economy_ttype['Activity_growth'], legendgroup=legend_name, name=legend_name, showlegend=False, line=dict(color=color3, dash='dot', width=2)), row=row_number, col=col_number, secondary_y=True)
                       
#             legend_name = 'Percentage_change_'+transport_type
#             fig.add_trace(go.Scatter(x=model_output_detailed_lv_passenger_economy_ttype['Date'], y=model_output_detailed_lv_passenger_economy_ttype['Percentage_change'], legendgroup=legend_name, name=legend_name, showlegend=False, line=dict(color=color4, dash='dot', width=2)), row=row_number, col=col_number, secondary_y=True)
...
# #sum activity by vehicle type and Date
# # model_output_concat_nz_sum = model_output_concat_nz.groupby(['Date', 'Vehicle Type', 'Dataset']).sum().reset_index()
# #sum activity by dataset and Date
# model_output_concat = model_output_concat.groupby(['Date', 'Economy', 'Dataset']).sum().reset_index()
# #plot activity for both datasets
# title = '9th Activity vs 8th activity'

# #
# #plot
# fig = px.line(model_output_concat, x="Date", y="Activity", color="Dataset", line_dash='Dataset', facet_col="Economy", facet_col_wrap=7, title=title)#, #facet_col="Economy",
