...
def logistic_fitting_function_handler(model_data,show_plots=False,matplotlib_bool=False, plotly_bool=False,ONLY_PASSENGER_VEHICLES=True,
    ...
    if old_activity_growth.groupby(['Date', 'Economy', 'Scenario', 'Transport Type']).size().max()!=1:
        # #it seems liek tis the base year thats causing this problem, which is weird. For now, just cause we in a rush, drop the earliest year and see if it fixes it:
        # old_activity_growth = old_activity_growth[old_activity_growth['Date'] != old_activity_growth['Date'].min()]
        # assert old_activity_growth.groupby(['Date', 'Economy', 'Scenario', 'Transport Type']).size().max()==1
