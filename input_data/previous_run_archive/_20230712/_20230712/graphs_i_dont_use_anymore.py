#Plot the ratio of BEVs and PHEVs to ICEs by Date, and economy
...
title = 'Ratio of BEVs to ICEs for each Date, by economy'
#plot
fig, ax = plt.subplots()
for key, grp in model_output_detailed_ratio_drive.groupby(['Economy']):
    ax = grp.plot(ax=ax, kind='line', x='Date', y='BEV_ICE_ratio', label=key)
plt.title(title)
plt.savefig('./plotting_output/diagnostics/{}.png'.format(title))

#plot
title='Ratio of PHEVs to ICEs for each Date, by economy'
fig, ax = plt.subplots()
for key, grp in model_output_detailed_ratio_drive.groupby(['Economy']):
    ax = grp.plot(ax=ax, kind='line', x='Date', y='PHEV_ICE_ratio', label=key)
plt.title(title)
plt.savefig('./plotting_output/diagnostics/{}.png'.format(title))
