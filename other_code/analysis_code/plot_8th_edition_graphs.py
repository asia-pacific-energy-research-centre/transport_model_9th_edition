...
model_8th_by_fuel_no_economy = model_8th_by_fuel_no_economy.groupby(['Year', 'Scenario', 'Medium', 'Transport Type', 'Vehicle Type', 'Drive', 'Fuel'], as_index=False).sum()

#keep pre 2050
model_8th_by_fuel_no_economy = model_8th_by_fuel_no_economy[model_8th_by_fuel_no_economy['Year']<=2050]
#%%
# #join onto original df
# model_output_8th_sum = model_output_8th_sum.merge(model_output_concat_sum_sales[['Year', 'Scenario', 'Transport Type', 'Vehicle Type', 'Drive', 'Sales share']], on=['Year', 'Scenario', 'Transport Type', 'Vehicle Type', 'Drive'], how='left')
#%%
...
model_output_concat_sum_sales = model_output_8th_sum_no_economy.groupby(['Year', 'Transport Type', 'Vehicle Type','Scenario', 'Drive'], as_index=False).sum()
...
model_output_concat_sum_sales['Sales_sum'] = model_output_concat_sum_sales.groupby(['Year','Transport Type','Vehicle Type', 'Scenario'])['Sales'].transform('sum')
...
model_output_8th_with_sales_share = model_output_8th_sum_no_economy.merge(model_output_concat_sum_sales, how='left', on=['Year', 'Transport Type', 'Vehicle Type','Scenario', 'Drive'])

#%%
#CALCUALTE SALES SHARE FOR ECONOMY
#sum stocks by year, vehicle type, drive type, dataset
model_output_concat_sum_sales_with_economy = model_output_8th_sum.groupby(['Year', 'Transport Type', 'Vehicle Type','Scenario', 'Drive'], as_index=False).sum()
#claulcate sales as teh schange in stocks each eyar
...
model_output_8th_sum_lv_passenger = model_output_8th_sum_no_economy_lv.groupby(['Year', 'Scenario', 'Medium', 'Transport Type', 'Vehicle Type', 'Drive'], as_index=False).sum()
...
model_output_8th_sum_lv_passenger = model_output_8th_sum_lv_passenger[model_output_8th_sum_lv_passenger['Year'] >= 2022]
...
#sum
model_output_8th_sum_lv_passenger = model_output_concat_sum_lv_passenger.groupby(['Year', 'Scenario', 'Medium', 'Transport Type', 'Vehicle Type', 'Drive'], as_index=False).sum()
    ...
    for economy in SCENARIO_ORDER:
        ...
        for drive_type in drive_list:
            ...
            if (col_number == 1) & (row_number == 1):#set the legend for the first subplot, and tehrefore all of the subplots
                ...
                fig.add_trace(go.Scatter(x=model_output_8th_plot_df_economy_drive['Year'], y=model_output_8th_plot_df_economy_drive['Stocks'],  legendgroup=legend_name, name=legend_name, line=dict(color=color, width=2, )), row=row_number, col=col_number, secondary_y=False)

                legend_name = drive_type + '_Vehicle_sales_share'
                fig.add_trace(go.Scatter(x=model_output_8th_plot_df_economy_drive['Year'], y=model_output_8th_plot_df_economy_drive['Sales share'], legendgroup=legend_name, name=legend_name, line=dict(color=color, dash='dot', width=2)), row=row_number, col=col_number, secondary_y=True)
            else:#legend is already set, so just add the traces with showlegend=False
                #create subplot for this economy AND DRIVE
                legend_name = drive_type + '_Stocks'
                fig.add_trace(go.Scatter(x=model_output_8th_plot_df_economy_drive['Year'], y=model_output_8th_plot_df_economy_drive['Stocks'],  legendgroup=legend_name, name=legend_name,showlegend=False, line=dict(color=color, width=2, )), row=row_number, col=col_number, secondary_y=False)

                legend_name = drive_type + '_Vehicle_sales_share'
                fig.add_trace(go.Scatter(x=model_output_8th_plot_df_economy_drive['Year'], y=model_output_8th_plot_df_economy_drive['Sales share'], legendgroup=legend_name, name=legend_name, showlegend=False, line=dict(color=color, dash='dot', width=2)), row=row_number, col=col_number, secondary_y=True)
...
for scenario in model_output_8th_sum_no_economy['Scenario'].unique():
    for transport_type in model_output_8th_sum_no_economy['Transport Type'].unique():
        ...
        model_output_8th_plot_df = model_output_8th_plot_df.groupby(['Year', 'Vehicle Type'])['Energy'].sum().reset_index()

        #plot
        fig = px.line(model_output_8th_plot_df, x="Year", y="Energy", color="Vehicle Type", title=title)#, #facet_col="Economy",
        ...
        model_output_8th_plot_df = model_output_8th_plot_df.groupby(['Year', 'Vehicle Type'])['Activity'].sum().reset_index()

        #plot
        fig = px.line(model_output_8th_plot_df, x="Year", y="Activity", color="Vehicle Type", title=title)#, #facet_col="Economy",
                    #category_orders={"Scenario": ["Reference", "Carbon Neutral"]})
        fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))#remove 'Economy=X' from titles
        fig.update_layout(
            font=dict(
...
title = 'Total activity for each year, vehicle type'
model_output_8th_sum_vtype = model_output_8th_sum_no_economy.groupby(['Year', 'Vehicle Type', 'Scenario'])['Activity'].sum().reset_index()

#plot
fig = px.line(model_output_8th_sum_vtype, x="Year", y="Activity", color="Vehicle Type", line_dash='Vehicle Type', facet_col="Scenario", facet_col_wrap=7, title=title, category_orders={"Scenario":['Carbon Neutrality','Reference']})#, #facet_col="Economy",
            #category_orders={"Scenario": ["Reference", "Carbon Neutral"]})
fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))#remove 'Economy=X' from titles
fig.update_layout(
    font=dict(
...
################################################################################################################################################################
#%%
#plot energy use for each economy for each year, by vehicle type, by transport type

title = 'Total activity use for each year, vehicle type, for freight'
#filter for freight
model_output_8th_sum_vtype = model_output_8th_sum_no_economy.loc[model_output_8th_sum_no_economy['Transport Type']=='freight']
model_output_8th_sum_vtype = model_output_8th_sum_vtype.groupby(['Year', 'Vehicle Type', 'Scenario'])['Activity'].sum().reset_index()

#plot
fig = px.line(model_output_8th_sum_vtype, x="Year", y="Activity", color="Vehicle Type", line_dash='Vehicle Type', facet_col="Scenario", facet_col_wrap=7, title=title, category_orders={"Scenario":['Reference', 'Carbon Neutrality']})#, #facet_col="Economy",
            #category_orders={"Scenario": ["Reference", "Carbon Neutral"]})
fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))#remove 'Economy=X' from titles
...
#%%
################################################################################################################################################################
#plot fuel use for for each year, road
title = 'Total fuel use for each year (road)'
...
model_output_with_fuels_plot = model_output_with_fuels_plot.groupby(['Year', 'Fuel','Scenario'])['Energy'].sum().reset_index()
...
#plot
fig = px.line(model_output_with_fuels_plot, x="Year", y="Energy", color="Fuel", facet_col="Scenario", facet_col_wrap=7, title=title, category_orders={"Scenario":['Reference', 'Carbon Neutrality']})#, #facet_col="Economy",
fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))#remove 'Economy=X' from titles
plotly.offline.plot(fig, filename='./plotting_output/8th_edition/' + title + '.html', auto_open=AUTO_OPEN_PLOTLY_GRAPHS)
# fig.write_image("./plotting_output/8th_edition/static/" + title + '.png', scale=1, width=2000, height=1500)

#plot fuel use for for each year, non-road
title = 'Total fuel use for each year (non-road)'
model_output_with_fuels_plot = model_8th_by_fuel_no_economy.copy()
model_output_with_fuels_plot = model_output_with_fuels_plot.loc[model_output_with_fuels_plot['Medium']!='road']
model_output_with_fuels_plot = model_output_with_fuels_plot.groupby(['Year', 'Fuel','Scenario'])['Energy'].sum().reset_index()
...
#plot activity each region for each year, by scenario by tranport type.
title = 'Total activity for each region by transport type, scenario'
model_output_8th_sum_vtype = model_8th_regions.groupby(['Year', 'Transport Type', 'Scenario', 'Region'])['Activity'].sum().reset_index()
...
#plot
fig = px.line(model_output_8th_sum_vtype, x="Year", y="Activity", color="Region", line_dash='Scenario', facet_col="Transport Type", facet_col_wrap=7, title=title, category_orders={"Scenario":['Reference', 'Carbon Neutrality']})#, #facet_col="Economy",
             #category_orders={"Scenario": ["Reference", "Carbon Neutral"]})
fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))#remove 'Economy=X' from titles
...
model_output_8th_sum_eff = model_output_8th_sum_no_economy.loc[model_output_8th_sum_no_economy['Year']==2050]
...
model_output_8th_sum_vtype = model_output_8th_sum_no_economy.groupby(['Year', 'Vehicle Type', 'Scenario']).sum().reset_index()
...
#group by year and vehicle type and sum energy again
model_output_8th_sum_vtype = model_output_8th_sum_vtype.groupby(['Year', 'Vehicle Type', 'Scenario'])['Energy'].sum().reset_index()
...
fig = px.area(model_output_8th_sum_vtype, x="Year", y="Energy", color="Vehicle Type", title=title, facet_col="Scenario", facet_col_wrap=1, color_discrete_map={'Shipping and aviation':'yellow', 'Heavy trucks':'red', 'Light duty vehicles':'pink', 'Other':'grey'}, category_orders={"Scenario":['Reference', 'Carbon Neutrality'], "Vehicle Type":['Other', 'Shipping and aviation', 'Heavy trucks', 'Light duty vehicles']})#, #facet_col="Economy",
fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))#remove 'Economy=X' from titles
...
model_output_8th_sum_vtype = model_8th_by_fuel_no_economy_emissions.groupby(['Year', 'Vehicle Type', 'Scenario']).sum().reset_index()
#combine vehicle types:
model_output_8th_sum_vtype.loc[model_output_8th_sum_vtype['Vehicle Type'].str.contains('ship'), 'Vehicle Type'] = 'Shipping and aviation'
model_output_8th_sum_vtype.loc[model_output_8th_sum_vtype['Vehicle Type'].str.contains('air'), 'Vehicle Type'] = 'Shipping and aviation'
model_output_8th_sum_vtype.loc[model_output_8th_sum_vtype['Vehicle Type'].str.contains('ht'), 'Vehicle Type'] = 'Heavy trucks'
...
#group by year and vehicle type and sum energy again
model_output_8th_sum_vtype = model_output_8th_sum_vtype.groupby(['Year', 'Vehicle Type', 'Scenario'])['Emissions'].sum().reset_index()
...
fig = px.area(model_output_8th_sum_vtype, x="Year", y="Emissions", color="Vehicle Type", title=title, facet_col="Scenario", facet_col_wrap=1, color_discrete_map={'Shipping and aviation':'yellow', 'Heavy trucks':'red', 'Light duty vehicles':'pink', 'Other':'grey'}, category_orders={"Scenario":['Reference', 'Carbon Neutrality'], "Vehicle Type":['Light duty vehicles', 'Heavy trucks', 'Shipping and aviation', 'Other'] })#, #facet_col="Economy",
fig.for_each_annotation(lambda a: a.update(text=a.text.split("=")[-1]))#remove 'Economy=X' from titles
...
#         model_output_8th_sum_vtype_melt = model_output_8th_sum_vtype.melt(id_vars=['Year', 'Economy', 'Drive'], value_vars=['Efficiency', 'New_vehicle_efficiency'], var_name='Measure', value_name='Efficiency')

#         fig = px.line(model_output_8th_sum_vtype_melt, x="Year", y="Efficiency", color="Drive", line_dash='Measure', facet_col="Economy", facet_col_wrap=7, title=title)#, #facet_col="Economy",
